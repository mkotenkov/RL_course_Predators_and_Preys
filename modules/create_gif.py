...
def get_text_info(r, info, env, model):
    ...
    if isinstance(model, RewardBasedModel):
        ...
        out.extend([
            ...
            dict(text=f'left:    {round(left, 2) if model.q_values is not None else None }',
            ...
            dict(text=f'right:    {round(right, 2) if model.q_values is not None else None }',
            ...
            dict(text=f'up:    {round(up, 2) if model.q_values is not None else None }',
            ...
            dict(text=f'down:    {round(down, 2) if model.q_values is not None else None }',
